...
/**
 * Add verified newsletter subscription to Google Sheet
 */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    try {
      ...
      console.error('[Verify Email API] Error adding to sheet:', sheetError)
      // Don't fail verification if sheet update fails - email is already verified
    }
    ...
    return res.redirect(302, '/verify-email?success=true')
